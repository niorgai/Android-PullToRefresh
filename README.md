为LoadingLayout添加方法`setBackgroundColorResource(int resId)`, 请在调用`setMode()`后通过`refreshView.getLoadingLayoutProxy().setLoadingLayoutBackground(R.color.red)`后调用,否则只有下拉的LoadingLayout可以设置背景.

---
# PLEASE NOTE, THIS PROJECT IS FORKED FROM [chrisbanes/Android-PullToRefresh](https://github.com/chrisbanes/Android-PullToRefresh), WHICH WAS DEPRECATED.
